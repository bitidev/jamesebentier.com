...
`universal.md` is the baseline best-practices doc for every agent that writes or reviews code (`builder`, `test`, `reviewer`) — the one guide `/adlc-init` always selects into `docs/code/`, regardless of stack. Because every selection always includes it, it also serves as the **single source** for how those agents select the *additional* stack-specific guide(s) to apply, and for the set of supported languages — so consumers reference the section below rather than re-enumerating the guides.

## How Agents Select the Right File

Every agent that writes or reviews code applies `universal.md` on every task. It then **also** applies the stack-specific file(s) matching the project's primary language/framework as recorded in `docs/code/conventions.md` (Technology Stack), which `/adlc-init` generates:

| Project's primary language/framework | Apply (in addition to `universal.md`) |
|---|---|
| Rails app | `ruby.md` + `rails.md` (Rails builds on Ruby) |
| Ruby (non-Rails) | `ruby.md` |
| TypeScript / JavaScript | `typescript-javascript.md` |
| Python | `python.md` |
| Kotlin on the JVM | `kotlin-jvm.md` |

When the project spans multiple stacks, apply the file for whichever language the file under change is written in.

This table is the **single source** for the supported-language set and the selection mapping. The `test` agent (`.claude/agents/test.md`), the `/adlc-init` skill's Language-Standards generation step, and a consumer's own `docs/code/conventions.md` all **reference** it rather than re-enumerating the guides, so no per-consumer edit is needed when the set changes — just this file.
...
- **[ruby.md](ruby.md)** — Ruby (nil safety, blocks/procs, enumerable, mixins, exceptions, metaprogramming, etc.)
- **[rails.md](rails.md)** — Rails (ActiveRecord/N+1, migrations, controllers, security, jobs, caching; builds on ruby.md)

Each language-specific file references these universal best practices and adds language-specific guidance.

### Universal vs project-specific

This file and its sibling language guides hold **universal** knowledge — language-agnostic or language-idiomatic best practices, standard patterns, common pitfalls, and testing approaches. **Project-specific** patterns (how THIS project structures code, its naming conventions, its error-handling and framework-integration patterns) live in `docs/code/`, not here.

### Adding a new language

1. Create `{language}.md` in this directory.
2. Include sections for common concerns (null safety, error handling, testing, etc.).
3. Add a selection row to the table above and a bullet to *Language-specific guides* above. Because every consumer references this section rather than re-listing the guides, no other file needs editing.
...
- [ ] Changes are focused (one feature/fix per PR/PR)
